# Figure 5: plasticity regimes, convergence basins, and line-attractor stability
import numpy as np
...
TICK_FS   = 8   # ticks (matches your ft-1 usage later)
LEGEND_FS = 9   # legends
...
mpl.rcParams.update({
    ...
    "font.family": "serif",       # LaTeX default vibe# optional but common:
    "mathtext.fontset": "cm",
...
tau_STDP = 120.0 / ms_per_sec          # (overwritten below by time-scale section; keeping here for clarity)
...
# Analytic line-attractor boundary (Q = 0 and R = b)
...
# ───────────────────────────────────────────────────────────────
# 2. ANALYTIC RATE FUNCTIONS R AND Q (not plotted directly)
# ───────────────────────────────────────────────────────────────
def R(wEE, wEI, w_IE):
...
# Compute the Jacobian of the plasticity vector field by finite differences.
...
# Parameterize the line attractor with a normalized one-dimensional coordinate.
...
"""
# Require both geometric proximity to the line and rE close to b.
...
# ───────────────────────────────────────────────────────────────
# 4. PLASTICITY SIMULATIONS
# ───────────────────────────────────────────────────────────────
# Retain the full rate and weight trajectories for plotting.
...
# Retain only the final state used by the convergence-grid calculation.
...
# ───────────────────────────────────────────────────────────────
# 5. PHASE-PLANE PLOTTING HELPER
# ───────────────────────────────────────────────────────────────
# Plot one bottom-row phase plane for the selected w_IE regime.
def placeholder_full_subplot(ax, aE, w_IE, label, w_IE_vals ):
    ...
    # 3) Fixed points via closed-form 2x2 solve
    ...
    # 4) Vector field (mask fully—no dots outside)
    ...
    # 8) Limits, ticks, labels (standardized fonts)
    ax.set_xlim(0, 0.003)
    ax.set_ylim(0, 0.003)
...
for i, w_IE in enumerate(w_IE_vals):
    ...
    # Evaluate the line-attractor eigenvalue for this regime.
    wEEval_rIzero = (aE * N * w_EX * w_IE + aI * w_IX) / (aI * N * w_IX)
    ...
    # Construct the top-left and top-middle regime summaries once.
    if i == 0:
        # Mesh for heatmap
        ...
        # Cross-section for a specific w_II (FIX w_II, SWEEP w_IE)
        ...
        # --- NEW: 2D heatmap of b_max(w_IE, w_II) on axes[0,1] ---
        # denom_2d > 0 → Regime 1 (full basin, b unbounded)
        # denom_2d < 0 → Regime 2 (finite b_max)
        denom_2d = N * w_ie - ratio * (1 + N * w_ii)

        b_max_map = np.full_like(denom_2d, np.nan, dtype=float)
        bounded_mask_2d = denom_2d < 0
        # finite b_max only where denom_2d < 0 (Regime 2)
        b_max_map[bounded_mask_2d] = -N * w_IX * aI / denom_2d[bounded_mask_2d]

        # Use NaNs for unbounded region (Regime 1) → single "bad" color
        b_max_masked = np.ma.masked_invalid(b_max_map)
        cmap_b = plt.cm.viridis.copy()
        # one uniform color for unbounded b (Regime 1)
    ...
    # Plot convergence with transparent blue/red
    wEE_grid, wEI_grid = grid_x, grid_y
    # 1. Oscillatory boundary
...
# 7. LEGENDS AND FINAL AXIS FORMATTING
ax = axs[0, 2]
...
# Axis labels/titles/ticks with standardized font sizes
