...
def OU_covariance_matrix(Sim_N, N_x, a, b, sigma_s, sigma_p, tau_ou, c):
    """Calculate the OU input covariance matrix used by the theory utilities."""
...
def calculate_CSD(Sim_N, N, N_x, W, W_x, tau_STDP, tau_r, tau_ou, c, a, b, sigma_p, sigma_s, lamb_e, lamb_i, normalize=True):
    """Integrate the two-population cross-spectrum against the STDP kernel."""
...
def generate_bernouilli_wx(init_weights, N, N_X, num_per_pool):
    """Generate sparse block-structured external connectivity."""
    p = 0.1
...
def generate_bernouilli_weight_array(init_weights, N_E, N_I):
    """Generate sparse recurrent E/I connectivity."""
    p = 0.1
...
def calculate_spike_count_covariance(spike_trains1, spike_trains2, window_size_ms, dt):
    """Calculate the average pairwise spike-count covariance."""
...
def generate_sep_poisson_neurons(base_ex, N_X, time_steps, dt, sigma_s, sigma_p, c_x):
    """Generate the existing shared-noise external-rate trace."""
    xi_s = np.random.normal(0, 1, size=len(time_steps))
    # Retained because this draw is part of the existing RNG sequence, although xi_p is not used below.
...
def generate_weight_array(init_weights, num_E, num_I):
    """Generate dense recurrent blocks with the existing small Gaussian weight variance."""
...
def generate_diagonal_wx(init_weights, N, N_X):
    """Generate the existing diagonal external-connectivity construction."""
...
def generate_block_wx(init_weights, N, N_X):
    """Generate dense block-structured external connectivity."""
    variances = [.00000001, .00000001]
    repeated_weights = np.zeros(shape=(N, N_X))
    shapes = N // 2
    ...
    plot = False
    if plot:
        plt.hist(repeated_weights.flatten(), alpha=.5, bins=50)
        plt.show()
    return repeated_weights


def calculate_firing_rate(N, spike_times, window_size, time_step, curr_t):
    """Calculate firing rates over the requested trailing window."""
