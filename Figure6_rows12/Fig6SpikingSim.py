"""Spiking-network simulation used for the Figure 6 sigma sweep.

The simulation equations, update order, random seeds, theory calculations,
returned arrays, and optional diagnostic plots are unchanged.
"""
...
def reciprocity_metrics(W, binary_threshold=None, indices=None):
    """
    Reciprocity metrics for an excitatory weight matrix W (N_E x N_E).
    - Excludes autapses.
    - If `indices` is provided (list/array), computes metrics on W[np.ix_(indices, indices)].
    - If `binary_threshold` is provided, also reports binary (A_ij) reciprocity above ER.
    Returns a dict with p_mean, R (weighted reciprocity), q_excess, q_rel, and (optionally) binary counterparts.
    """
    W = np.asarray(W, dtype=float)
    if indices is not None:
        W = W[np.ix_(indices, indices)]
    assert W.ndim == 2 and W.shape[0] == W.shape[1], "W must be square"

    N = W.shape[0]
    # Exclude autapses without modifying the caller's matrix
    Wn = W.copy()
    np.fill_diagonal(Wn, 0.0)

    # Mean E-to-E weight over off-diagonal entries
    off = ~np.eye(N, dtype=bool)
    p = Wn[off].mean() if off.sum() else np.nan

    # Weighted reciprocity over unordered neuron pairs
    upper = np.triu(np.ones((N, N), dtype=bool), k=1)
    R = (Wn * Wn.T)[upper].mean() if upper.sum() else np.nan

    # Excess reciprocity above the independent-weight baseline
    q_excess = R - p**2
    q_rel = (R / (p**2) - 1.0) if p > 0 else np.nan

    out = dict(p_mean=p, reciprocity_weighted=R, q_excess=q_excess, q_rel=q_rel)

    # Optional: binary reciprocity (set A_ij = 1 if W_ij > threshold)
    if binary_threshold is not None:
        A = (Wn > binary_threshold).astype(float)
        p_conn = A[off].mean()
        R_bin = (A * A.T)[upper].mean()
        q_excess_bin = R_bin - p_conn**2
        q_rel_bin = (R_bin / (p_conn**2) - 1.0) if p_conn > 0 else np.nan
        out.update(dict( p_conn=p_conn, reciprocity_binary=R_bin, q_excess_binary=q_excess_bin, q_rel_binary=q_rel_bin ))
    return out

# ---------------------------------------------------------------------------
# Figure 6 simulation
def spiking_sim(normalize, axs,seed,c_x,sigma,ctt,tau_rprim, tau_r, tau_ou,tau_STDP, tau_wee, tau_wei, N_E,N_I,T, dt, w_EE,w_EI,w_IE,w_II, OnOff=False):
    ...
    Wxm=tau_rprim*np.array([20.,20. ])/np.sqrt(N_E)
    ...
    # -----------------------------------------------------------------------
    # Time stepping: correlated OU input, spikes, plasticity, and theory
    time_reduced = time[::skip]
    q_val = np.zeros(shape=len(time_reduced))
    for t in range(len(time)-1):
        ...
        if t%skip==0:
            q_val[t//skip] = reciprocity_metrics(W[:N_E,:N_E])['q_rel']
        ...
        if t %10000==0:
            print('time',time[t],'b',b,'THEORY LAM',tr[0],'poisson lamb',np.mean(Lam[:N_E]))
    ...
    if plot_theory:
        print('hellow after plot theory')
        theory_lamb = np.vstack([theory_rates[0,:],theory_rates[1,:]])
        print('hellow after plot theory print')
    else:
    ...
    return time ,track_W,track_W_mean, track_mean_rates,time_reduced,track_lam_E,track_lam_I, spike_trains_E, spike_trains_I,theory_weights[0,:],theory_weights[1,:],theory_lamb,balance_index, b,q_val
