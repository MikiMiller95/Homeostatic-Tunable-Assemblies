...
# OU-process covariance matrix in the Fourier domain.
def OU_covariance_matrix(Sim_N, N_x,a,b, sigma_s,sigma_p,tau_ou,c,omega):
    ...
    cov_matrix[np.arange(N_x//2), np.arange(N_x//2)] += a/Sim_N #+ cov_matrix[np.arange(N_x//2), np.arange(N_x//2)]
    cov_matrix[N_x//2+np.arange(N_x//2), N_x//2+np.arange(N_x//2)] += b/Sim_N #+ cov_matrix[N_x//2+np.arange(N_x//2), N_x//2+np.arange(N_x//2)]
    cov_matrix=Sim_N*cov_matrix*Sim_N

    return cov_matrix


# Integrate the recurrent and external cross spectrum against the STDP kernel.
def calculate_CSD(Sim_N,N,N_x,W, W_x,tau_STDP, tau_r,tau_ou,c,a,b,sigma_p,sigma_s,lamb_e,lamb_i,normalize=True):
    # Calculate the cross spectrum for each frequency
    omega=np.linspace(-2500,2500,50000)
...
# Optional sparse external and recurrent connectivity helpers.
...
def calculate_spike_count_covariance(spike_trains1,spike_trains2, window_size_ms, dt):
    ...
    # Calculate spike counts for each neuron in each window
    ...
    # Calculate mean spike counts for each neuron
...
def generate_sep_poisson_neurons(base_ex,N_X, time_steps, dt, sigma_s, sigma_p, c_x):
    # Generate shared white noise (mean 0, variance 1)
    xi_s = np.random.normal(0, 1, size=len(time_steps))
    # Generate private white noise for each neuron (mean 0, variance 1)
    xi_p = np.random.normal(0, 1, size=(N_X, len(time_steps)))
    # Calculate firing rates for each neuron
    lambda_xx = base_ex + c_x * sigma_s * xi_s  #+ (1 - c_x) * sigma_p * xi_p
...
# Dense block-structured external-input matrix.
...
# Assuming spike_times is your N_I x Time array
def calculate_firing_rate(N,spike_times, window_size, time_step,curr_t):
    firing_rate = np.zeros((N))

    # Loop through each neuron
    for i in range(N):
        # Loop through each time point with the specified time step
        # Define the window range
        start = int( curr_t - window_size )
        end = int(curr_t)
        # Count the number of spikes in the window
