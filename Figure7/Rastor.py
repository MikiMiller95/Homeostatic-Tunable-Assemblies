...
for sigma in sigmas:
    ...
    df_spikes_X.to_csv(f'{loc}spike_trains_X_Ne_{N_E}_T{T}_wee{w_EE * factor}_wei{w_EI * factor}_wii{w_II * factor}_wie{w_IE * factor}_sigma{sigma}_cx{c_x}_taur{tau_r}_tauSTDP{tau_STDP}_tauou{tau_ou}_tauwee{tau_wee}_tauwei{tau_wei}_seed{seed}.csv', index=False)
        ...
        # track_lam_E and track_lam_I each have shape (track_num_n, # timesteps)
    df_lamE = pd.DataFrame(track_lam_E)
    df_lamE.to_csv(f'{loc}tracked_lamE_Ne_{N_E}_T{T}_wee{w_EE}_wei{w_EI}_wii{w_II}_wie{w_IE}_sigma{sigma}_cx{c_x}_'f'taur{tau_r}_tauSTDP{tau_STDP}_tauou{tau_ou}_tauwee{tau_wee}_tauwei{tau_wei}_seed{seed}.csv',index=False)

    df_lamI = pd.DataFrame(track_lam_I)
    df_lamI.to_csv(f'{loc}tracked_lamI_Ne_{N_E}_T{T}_wee{w_EE}_wei{w_EI}_wii{w_II}_wie{w_IE}_sigma{sigma}_cx{c_x}_'f'taur{tau_r}_tauSTDP{tau_STDP}_tauou{tau_ou}_tauwee{tau_wee}_tauwei{tau_wei}_seed{seed}.csv',index=False)
