...
def OU_covariance_matrix(Sim_N, N_x, a, b, sigma_s, sigma_p, tau_ou, c):
    """Construct the external-input covariance matrix used by calculate_CSD."""
...
def calculate_CSD(Sim_N, N, N_x, W, W_x, tau_STDP, tau_r, tau_ou, c, a, b, sigma_p, sigma_s, lamb_e, lamb_i, normalize=True):
    """Calculate the covariance contribution to the plasticity dynamics."""
    lambda_cov = OU_covariance_matrix(Sim_N, N_x, a, b, sigma_s, sigma_p, tau_ou, c)

    # Build the population-rate covariance and frequency grid.
    D = np.zeros(shape=(N, N))
    D[np.arange(N // 2), np.arange(N // 2)] = lamb_e
    D[N // 2 + np.arange(N // 2), N // 2 + np.arange(N // 2)] = lamb_i

    omega = np.linspace(-10000, 10000, 100000)
    domega = omega[1] - omega[0]
    omega = omega[np.newaxis, np.newaxis, :]

    # Repeat the matrices over frequency so the original vectorized operations
    # can be evaluated without changing their order.
    Id = np.eye(N)
    ...
    # Construct the synaptic transfer functions.
    tau_ri = 2 * tau_r
    if normalize:
...
def generate_bernouilli_wx(init_weights, N, N_X, num_per_pool):
    """Generate sparse Bernoulli external-input weights."""
    p = 0.1
...
def generate_bernouilli_weight_array(init_weights, N_E, N_I):
    """Generate the sparse Bernoulli recurrent weight matrix."""
    p = 0.1
...
def generate_weight_array(init_weights, num_E, num_I):
    """Generate the dense recurrent weight matrix used by Figure 3 simulations."""
    variances = [0.000000001, 0.000000001, 0.000000001, 0.000000001]
...
def generate_diagonal_wx(init_weights, N, N_X):
    """Generate diagonal external-input weights."""
    variances = [0.000000001, 0.000000001]
...
def generate_block_wx(init_weights, N, N_X):
    """Generate the block-structured external-input weights used in the simulations."""
    variances = [0.000000001, 0.000000001]
    repeated_weights = np.zeros(shape=(N, N_X))
    shapes = N // 2
...
# -----------------------------------------------------------------------------
# Spike-train utilities retained from the original helper file
# -----------------------------------------------------------------------------
def calculate_spike_count_covariance(spike_trains1, spike_trains2, window_size_ms, dt):
    """Calculate the average spike-count covariance between two neuron groups."""
...
def generate_sep_poisson_neurons(base_ex, N_X, time_steps, dt, sigma_s, sigma_p, c_x):
    """Generate the shared component of the external Poisson rate process."""
...
def calculate_firing_rate(N, spike_times, window_size, time_step, curr_t):
    """Calculate each neuron's firing rate over the requested trailing window."""
